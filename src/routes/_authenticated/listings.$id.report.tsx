...
function ListingReport() {
  ...
  if (!data?.listing)
    return <div className="container-luxe py-10 text-muted-foreground">Listing not found.</div>;
  ...
  pageViews.forEach((e) => {
    if (e.visitor_hash) visitCounts.set(e.visitor_hash, (visitCounts.get(e.visitor_hash) ?? 0) + 1);
  });
  ...
  const untitledInstalled =
    !!tracking?.untitled_script ||
    !!(tracking?.custom_header_script && /untitled/i.test(tracking.custom_header_script));
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `listing-${listing.slug}-report.csv`;
    a.click();
    ...
    <div className="container-luxe py-10">
      ...
      <Card className="p-8 mb-6">
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div>
            ...
            <div className="flex items-center gap-3 mt-4">
              {listing.price && (
                <span className="font-mono text-lg">${Number(listing.price).toLocaleString()}</span>
              )}
              <Badge
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="sm" disabled>
              <FileText className="h-3 w-3 mr-1" /> Seller Report PDF (coming soon)
            </Button>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 mt-8 pt-6 border-t">
          <div>
            ...
            <div className="flex gap-2">
              <code className="flex-1 text-xs bg-muted/40 px-3 py-2 rounded truncate">
                {brandedUrl}
              </code>
              <Button size="sm" variant="outline" onClick={() => copy(brandedUrl, "Branded URL")}>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => copy(unbrandedUrl, "Unbranded URL")}
              >
                <Copy className="h-3 w-3" />
              </Button>
              <a href={unbrandedUrl} target="_blank" rel="noreferrer">
                <Button size="sm" variant="outline">
                  <ExternalLink className="h-3 w-3" />
                </Button>
              </a>
      ...
      <Card className="p-6 mb-8 bg-muted/20">
        ...
        <p className="text-sm leading-relaxed text-muted-foreground">
          ...
          for portal and MLS traffic, while the branded page can be used for email, social, QR
          codes, and direct sharing. Use the activity breakdown below to see where engagement is
          coming from and how visitors are interacting with the tour.
        </p>
      </Card>
      ...
      <Card className="p-6 mb-8">
        ...
        <ul className="grid md:grid-cols-2 gap-3 text-sm">
          <li className="flex justify-between border-b pb-2">
            <span>Tracking enabled on branded pages</span>
            <Badge variant={trackingBranded ? "default" : "secondary"}>
              {trackingBranded ? "Yes" : "No"}
            </Badge>
          </li>
          <li className="flex justify-between border-b pb-2">
            <span>Tracking enabled on unbranded pages</span>
            <Badge variant={trackingUnbranded ? "default" : "secondary"}>
              {trackingUnbranded ? "Yes" : "No"}
            </Badge>
          </li>
          <li className="flex justify-between border-b pb-2">
            <span>Audience activation tag installed</span>
            <Badge variant={untitledInstalled ? "default" : "secondary"}>
              {untitledInstalled ? "Yes" : "No"}
            </Badge>
          </li>
          <li className="flex justify-between border-b pb-2">
            <span>Last page view recorded</span>
            <span className="text-muted-foreground">
              {lastView ? new Date(lastView).toLocaleString() : "Never"}
            </span>
          </li>
        </ul>
      </Card>
