...
function TrackingVerify() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [{ data: t }, { data: l }] = await Promise.all([
        ...
        supabase.from("listings").select("slug").eq("company_id", companyId).eq("mls_number", DEMO_MLS).eq("status", "active").order("created_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      setTracking(t);
  ...
  const hasAnyScript = !!(tracking && (tracking.custom_header_script || tracking.custom_footer_script || tracking.untitled_script || tracking.ga_script || tracking.meta_pixel_script));
  const brandedOn = !!tracking?.enable_branded_tracking;
  const unbrandedOn = !!tracking?.enable_unbranded_tracking;

  function copy(v: string) { void navigator.clipboard.writeText(v); toast.success("Copied"); }
  ...
  return (
    <div className="container-luxe py-10 max-w-3xl">
      ...
      <Card className="p-5 mb-6">
        <p className="text-xs uppercase tracking-widest text-muted-foreground mb-3">Current settings</p>
      ...
      <Card className="p-5 mb-6 space-y-4">
        <p className="text-xs uppercase tracking-widest text-muted-foreground">Public URLs to verify</p>
        {slug ? (
          <>
            <div>
              ...
              <div className="flex gap-2">
                ...
                <Button size="icon" variant="outline" onClick={() => copy(branded)}><Copy className="h-4 w-4" /></Button>
                <Button size="icon" variant="outline" asChild><a href={branded} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4" /></a></Button>
              </div>
            </div>
            <div>
          ...
          <p className="text-sm text-muted-foreground">
            No demo listing found. <Link to="/test-center" className="underline">Create one</Link> to get verifiable URLs.
          </p>
        )}
      </Card>

      <Card className="p-5">
        <p className="text-xs uppercase tracking-widest text-muted-foreground mb-3">How to verify</p>
        <ol className="list-decimal list-inside space-y-2 text-sm text-muted-foreground">
          <li>Open <strong>Tracking</strong> and click <strong>Insert Test Tracking Script</strong>, then Save.</li>
          <li>Enable the toggles for branded and/or unbranded pages, then Save.</li>
          <li>Open each public URL above in a <strong>Guest / Incognito</strong> browser window.</li>
          <li>Open DevTools → Console. You should see: <code className="font-mono text-foreground">SmartTourOS tracking script fired</code></li>
          <li>In the Console, type <code className="font-mono text-foreground">window.smartTourTrackingTest</code> — it should return <code className="font-mono text-foreground">true</code>.</li>
          <li>Confirm the page still renders normally and a page_view is recorded (see <Link to="/test-center" className="underline">MVP Test Center</Link>).</li>
        </ol>
      </Card>
    </div>
  );
